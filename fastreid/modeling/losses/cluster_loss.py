...
def intraCluster(feature, domain_ids, margin=0.1):
    ...
    for i in range(3):
        ...
        # print("domain_ids", domain_ids.shape, domain_ids)
        ...
        domain_ids_list = domain_ids==i
        if domain_ids_list.shape == ():
            domain_ids_list = domain_ids_list.reshape(1)
        loss += (mindspore.ops.relu(mindspore.ops.norm(feature[domain_ids_list] - feature[domain_ids_list].mean(), 2, 1) - margin) ** 2).mean()
...
def interCluster(feature, domain_ids, margin=0.3):
    ...
    for i in range(3):
        ...
        if domain_ids_list.shape == ():
            domain_ids_list = domain_ids_list.reshape(1)
        if domain_ids_list.sum().item() == 0:
            continue
        candidate_list.append(feature[domain_ids_list].mean())
